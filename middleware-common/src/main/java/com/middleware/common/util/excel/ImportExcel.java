...
public class ImportExcel {
     ...
     * @param headerNum 标题行号，数据行号=标题行号+1
     */
    public ImportExcel(String fileName, InputStream is, int headerNum) throws InvalidFormatException, IOException {
                ...
                while (row.hasNext()) {
                    ...
                    while (cellBody.hasNext()) {
                        ...
                        if (cell.getColumnIndex() >= titleRow.getLastCellNum()
                                || StringUtils.isEmpty(getCellFormatValue(cell))) {
                            break;
                        }
                        // 这里得到此列的对应的标题
                        ...
                        if (fieldMap.containsKey(titleString)) {
                            ...
                            if ("class java.lang.String".equals(xclass)) {
                                ...
                                if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
                                    setMethod.invoke(tObject, Integer
                                            .valueOf(String.valueOf((int) cell.getNumericCellValue())));
                                } else if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
                                ...
                                if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
                                    setMethod.invoke(tObject,
                                            Long.valueOf(String.valueOf((long) cell.getNumericCellValue())));
                                } else if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
                                ...
                                if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
                                    setMethod.invoke(tObject,
                                            new BigDecimal(String.valueOf((double) cell.getNumericCellValue())));
                                } else if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
                                ...
                                if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
                                    setMethod.invoke(tObject,
                                            Integer.valueOf(String.valueOf((int) cell.getNumericCellValue())));
                                } else if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
                                    setMethod.invoke(tObject, Integer.valueOf(cell.getStringCellValue()));
                                }
                    ...
                    ReflectionUtil.invokeSetter(tObject, "gradeName", sheet.getSheetName());
                    ReflectionUtil.invokeSetter(tObject, "gradeIndex", index + 1);
            ...
            for (int index = 0; index < sheetList.size(); index++) {
                ...
                while (row.hasNext()) {
                    ...
                    while (cellBody.hasNext()) {
                        ...
                        // 这里得到此列的对应的标题
                        String titleString = titleRow.getCell(cell.getColumnIndex()).getStringCellValue();
                        // 如果这一列的标题和类中的某一列的Annotation相同，那么则调用此类的的set方法，进行设值
                        if (fieldMap.containsKey(titleString)) {
                            ...
                            if ("class java.lang.String".equals(xclass)) {
                                ...
                                if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
                                    setMethod.invoke(tObject,
                                            Double.valueOf(String.valueOf((Double) cell.getNumericCellValue())));
                                } else if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
                                    setMethod.invoke(tObject, Integer.valueOf(cell.getStringCellValue()));
                                }
                            } else if ("class java.util.Date".equals(xclass)) {
